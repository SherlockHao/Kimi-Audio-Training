...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, raw_data_list, whisper_model, text_tokenizer, max_len: int, kimia_token_offset: int):
        ...
        # Remove the cached_data_dict as we won't use unlimited caching
    ...
    def extract_whisper_feat(self, wav: str):
        """Extract whisper features with memory-efficient loading."""
        # Load audio with memory mapping when possible
        try:
            # Use mono=True to reduce memory, normalize to prevent overflow
            wav_data, _ = librosa.load(wav, sr=16000, mono=True, res_type='kaiser_fast')
            
            # Convert to float32 if needed (librosa returns float64 by default)
            if wav_data.dtype != np.float32:
                wav_data = wav_data.astype(np.float32)
            
            return wav_data
        except Exception as e:
            print(f"Error loading audio file {wav}: {e}")
    ...
    # REMOVED @lru_cache decorator - this was causing memory issues!
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            # Return a dummy sample on error to prevent training from crashing
            return self._get_dummy_sample()
    
    def _get_dummy_sample(self):
        """Return a dummy sample in case of loading errors."""
        dummy_len = 10  # Short dummy sequence
        return dict(
            input_ids=torch.zeros(1, dummy_len, dtype=torch.long),
            text_input_ids=torch.zeros(1, dummy_len, dtype=torch.long),
            whisper_input_feature=[np.zeros((160000,), dtype=np.float32)],  # 10 seconds of silence
            is_continuous_mask=torch.zeros(1, dummy_len, dtype=torch.bool),
            labels=(
                torch.zeros(1, dummy_len, dtype=torch.long),
                torch.zeros(1, dummy_len, dtype=torch.long),
                torch.zeros(1, dummy_len, dtype=torch.bool),
                torch.zeros(1, dummy_len, dtype=torch.bool),
            ),
        )
